...
-(void)loadHtmlFile:(NSString*)htmlName{
	...
	NSString *detailHtmlPath = [[NSBundle mainBundle] pathForResource:htmlName ofType:@"html"];
    NSURL *url = [NSURL fileURLWithPath:detailHtmlPath isDirectory:NO];
    [webView loadRequest:[NSURLRequest requestWithURL:url]];
}

- (void)loadOtherFile:(NSString *)fileName type:(NSString *)fileType
{
    NSString *detailPath = [[NSBundle mainBundle] pathForResource:fileName ofType:fileType];
    NSURL *url = [NSURL fileURLWithPath:detailPath isDirectory:NO];
    [webView loadRequest:[NSURLRequest requestWithURL:url]];
}
