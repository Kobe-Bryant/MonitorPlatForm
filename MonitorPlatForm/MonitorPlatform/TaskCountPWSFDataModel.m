...
@implementation TaskCountPWSFDataModel
...
-(void)requestDataWithFromDate:(NSString*)from andEndDate:(NSString*)end andDepID:(NSString*)depID{
  ...
  //  if(isDataRequested){
   ...
   // }
    ...
    self.endDate = end;
    isLoading = YES;
    NSString *param = [WebServiceHelper createParametersWithKey:@"StartDate" value:from,
                       @"EndDate",end,@"strDept",depID,nil];
...
{
    if ([elementName isEqualToString:@"TaskCountPWSF_New_ExResult"])
...
{
    if (isGotJsonString && [elementName isEqualToString:@"TaskCountPWSF_New_ExResult"]) 
    {
...
- (void)parserDidEndDocument:(NSXMLParser *)parser
{
    ...
    NSMutableArray *dealAry = [NSMutableArray arrayWithCapacity:50];
    for (NSDictionary *tmpDic in resultDataAry)
    {
        NSString *person = [tmpDic objectForKey:@"执法人员"];
        if ([person isEqualToString:@"系统管理员"])
            continue;
        else
        {
            [dealAry addObject:tmpDic];
        }
    }
    ...
    [self.resultDataAry addObjectsFromArray:dealAry];*/
    self.resultDataAry = dealAry;
