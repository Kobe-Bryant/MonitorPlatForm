...
-(void)drawRect:(CGRect)_rect{
	...
	CGContextSetFillColorWithColor(context, LIGHT_BLUE_UICOLOR2.CGColor);
...
-(void)drawColumn:(CGContextRef)context rect:(CGRect)_rect{
	...
	for(NSArray *g in groupData){
		...
		if (bGrouped) {
			...
			[str drawInRect:txtRect withFont:[UIFont fontWithName:@"Helvetica" size:12] lineBreakMode:UILineBreakModeWordWrap alignment:UITextAlignmentCenter];
...
-(void)calcScales:(CGRect)_rect{
    ...
    if (columnWidth > ((_rect.size.width - MARGIN_LEFT * 2)/8))
        columnWidth = (_rect.size.width - MARGIN_LEFT * 2)/8;
    groupWidth = (_rect.size.width - MARGIN_LEFT * 2)/groupCount;
    if (groupWidth > ((_rect.size.width - MARGIN_LEFT * 2)/8))
        groupWidth = (_rect.size.width - MARGIN_LEFT * 2)/8;
    ...
    self.timerDraw = [NSTimer scheduledTimerWithTimeInterval:0.1 target:self selector:@selector(animationTurn) userInfo:nil repeats:YES];
