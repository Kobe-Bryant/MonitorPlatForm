...
@implementation TaskChildYJDataModel
...
-(void)requestDataWithFromDate:(NSString*)from andEndDate:(NSString*)end andDepID:(NSString*)depID{
    ...
    NSString *param = [WebServiceHelper createParametersWithKey:@"StartDate" value:from,@"EndDate",end,@"orgid",@"",@"strDept",depID,nil];
    self.fromDate = from;
    self.endDate = end;
...
{
    if ([elementName isEqualToString:@"TaskCountYJ_New_ExResult"])
...
{
    if (isGotJsonString && [elementName isEqualToString:@"TaskCountYJ_New_ExResult"]) 
    {
