...
- (void) layoutSubviews
{
	...
	if(self.badgeString)
	{
		...
		CGSize badgeSize = [self.badgeString sizeWithFont:[UIFont boldSystemFontOfSize: 17]];
		CGRect badgeframe = CGRectMake(self.contentView.frame.size.width - (badgeSize.width + 25),
                                (CGFloat)round((self.contentView.frame.size.height - 30) / 2),
                                badgeSize.width + 13,
                                30);
		...
		if(self.badgeColor)
			...
			self.badge.badgeColor = CELL_HEADER_COLOR;
	}
	else
	{
		[self.badge setHidden:YES];
