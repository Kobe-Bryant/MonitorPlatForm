...
@implementation TaskCountXZCFDataModel
...
-(void)requestDataWithFromDate:(NSString*)from andEndDate:(NSString*)end andDepID:(NSString*)depID{
    ...
    [param setObject:@"GET_XZCF_XZCFTJSL" forKey:@"service"];
    ...
    if (webservice) {
        [webservice cancel];
        [webservice initWithUrl:urlStr andParentView:parentController.view delegate:self];
    }
    else{
...
-(void)processWebData:(NSData*)webData{
    ...
    if (bSuccess)   
    {/*
        NSMutableArray *dataAry = [NSMutableArray arrayWithCapacity:5];
        
        for (NSDictionary *tmpDic in resultAry)
        {
            NSMutableDictionary *dataDic = [NSMutableDictionary dictionaryWithDictionary:tmpDic];
            int wcs = [[tmpDic objectForKey:@"已完成数"] intValue];
            int cswcs = [[tmpDic objectForKey:@"超时完成数"] intValue];
            int zcwcs = wcs-cswcs;
            [dataDic setObject:[NSNumber numberWithInt:zcwcs] forKey:@"正常完成数"];
            
            int rwzs = [[tmpDic objectForKey:@"任务总数"] intValue];
        ...
        if(displayTableView.hidden == YES)
        {
