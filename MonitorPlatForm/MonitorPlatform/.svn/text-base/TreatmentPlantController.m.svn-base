...
#import "TreatmentPlantController.h"
...
@implementation TreatmentPlantController
...
- (void)getWebData
{
    ...
    self.webHelper = [[[WebServiceHelper alloc] initWithUrl:strUrl method:@"RetrieveWastewaterTreatmentPlantRecord" nameSpace:@"http://tempuri.org/" parameters:param delegate:self] autorelease];
...
{
    if ([elementName isEqualToString:@"RetrieveWastewaterTreatmentPlantRecordResult"])
        isGotJsonString = YES;
}
...
{
    if (isGotJsonString && [elementName isEqualToString:@"RetrieveWastewaterTreatmentPlantRecordResult"]) 
    {
...
- (void)parserDidEndDocument:(NSXMLParser *)parser
{
    if ([curParsedData length] > 0) {
        ...
        NSString *czwt = [[infoDic objectForKey:@"存在的问题及整改要求"] stringByReplacingOccurrencesOfString:@"<br/>" withString:@"\n"];
        ...
        NSString *htmlStr = [HtmlTableGenerator getContentWithTitle:@"污水处理厂笔录" andParaMeters:adjustDic andServiceName:@"RetrieveWastewaterTreatmentPlantRecord"];
        myWebView.dataDetectorTypes = UIDataDetectorTypeNone;
        
        [myWebView loadHTMLString:htmlStr baseURL:[[NSBundle mainBundle] bundleURL]];
        
    } else {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示" message:@"没有污水处理厂笔录信息，请检查数据库" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
        [alert show];
        [alert release];
    }
}
