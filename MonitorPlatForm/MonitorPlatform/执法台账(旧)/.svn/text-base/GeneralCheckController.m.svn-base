...
#import "GeneralCheckController.h"
...
@implementation GeneralCheckController
...
- (void)getWebData
{
    ...
    self.webHelper = [[[WebServiceHelper alloc] initWithUrl:strUrl method:@"RetrieveGeneralCheckRecord" nameSpace:@"http://tempuri.org/" parameters:param delegate:self] autorelease];
...
{
    if ([elementName isEqualToString:@"RetrieveGeneralCheckRecordResult"])
        isGotJsonString = YES;
}
...
{
    if (isGotJsonString && [elementName isEqualToString:@"RetrieveGeneralCheckRecordResult"]) 
    {
...
- (void)parserDidEndDocument:(NSXMLParser *)parser
{
    if ([curParsedData length] > 0) {
        ...
        NSString *xcjcqk = [infoDic objectForKey:@"现场检查情况"];
        xcjcqk = [xcjcqk stringByReplacingOccurrencesOfString:@"<br/>" withString:@"\n"];
        ...
        [adjustDic setObject:xcjcqk forKey:@"现场检查情况"];
        
        NSString *htmlStr = [HtmlTableGenerator getContentWithTitle:@"其他现场检查笔录" andParaMeters:adjustDic andServiceName:@"RetrieveGeneralCheckRecord"];
    ...
    {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示" message:@"没有其它现场检查笔录信息，请检查数据库" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
        [alert show];
        [alert release];
    }
}
