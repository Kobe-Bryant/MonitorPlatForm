...
- (void)getWebData
{
    ...
    self.webHelper = [[[WebServiceHelper alloc] initWithUrl:strUrl method:@"RetrievePowerStationRecord" nameSpace:@"http://tempuri.org/" parameters:param delegate:self] autorelease];
...
{
    if ([elementName isEqualToString:@"RetrievePowerStationRecordResult"])
        isGotJsonString = YES;
}
...
{
    if (isGotJsonString && [elementName isEqualToString:@"RetrievePowerStationRecordResult"]) 
    {
...
- (void)parserDidEndDocument:(NSXMLParser *)parser
{
    if ([curParsedData length] > 0) {
        ...
        NSString *htmlStr = [HtmlTableGenerator getContentWithTitle:@"电厂检查笔录" andParaMeters:adjustDic andServiceName:@"RetrievePowerStationRecord"];
        myWebView.dataDetectorTypes = UIDataDetectorTypeNone;
        
        [myWebView loadHTMLString:htmlStr baseURL:[[NSBundle mainBundle] bundleURL]];
        
    } else {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示" message:@"没有电厂笔录信息，请检查数据库" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
        [alert show];
        [alert release];
    }
}
