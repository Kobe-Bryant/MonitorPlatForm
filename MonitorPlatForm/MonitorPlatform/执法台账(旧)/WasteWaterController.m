...
@implementation WasteWaterController
...
- (void)getWebData
{
    ...
    self.webHelper = [[[WebServiceHelper alloc] initWithUrl:strUrl method:@"RetrieveWastewaterRecord" nameSpace:@"http://tempuri.org/" parameters:param delegate:self] autorelease];
...
{
    if ([elementName isEqualToString:@"RetrieveWastewaterRecordResult"])
        isGotJsonString = YES;
}
...
{
    if (isGotJsonString && [elementName isEqualToString:@"RetrieveWastewaterRecordResult"]) 
    {
...
- (void)parserDidEndDocument:(NSXMLParser *)parser
{
    if ([curParsedData length] > 0) {
        ...
        NSString *htmlStr = [HtmlTableGenerator getContentWithTitle:@"废水检查笔录" andParaMeters:adjustDic andServiceName:@"RetrieveWastewaterRecord"];
        myWebView.dataDetectorTypes = UIDataDetectorTypeNone;
        
        [myWebView loadHTMLString:htmlStr baseURL:[[NSBundle mainBundle] bundleURL]];
    } else {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示" message:@"没有废水笔录信息，请检查数据库" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
        [alert show];
        [alert release];
    }
}
